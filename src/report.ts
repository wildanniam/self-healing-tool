import { libraryInspection } from './report-adapter.js';
import { renderLibraryReport } from './library-report.js';
import type { RunAudit } from './types.js';
...
export interface ReportOptions { audit?: RunAudit; language?: 'en' | 'id' }
export function renderReport(run: Run, price?: PriceAssumption, options: ReportOptions = {}): string {
  const view = reportView(run, price);
  return renderLibraryReport(view, libraryInspection(run, view, options.audit), options.language);
}
export async function writeReport(run: Run, options: { directory: string; price?: PriceAssumption; audit?: RunAudit; language?: 'en' | 'id'; diagnostics?: readonly { eventId: string; rawDom: string }[] }): Promise<string> {
  if (!/^[a-f0-9-]{36}$/.test(run.id)) throw new Error('Invalid run ID');
  const html = renderReport(run, options.price, options); // Validate evidence identity before writing.
  ...
  await writeFile(join(directory, 'report.html'), html, { flag: 'wx', mode: 0o600 });
  if (options.audit) await writeFile(join(directory, 'SENSITIVE-local-audit.json'), JSON.stringify(libraryInspection(run, reportView(run, options.price), options.audit), null, 2), { flag: 'wx', mode: 0o600 });
