...
const out = resolve(process.env.D30_SYNTHETIC_OUTPUT ?? join(root, 'output/d30/synthetic-v1'));
const toolDir = resolve(process.env.D30_TOOL_DIST ?? join(root, 'dist'));
const baselineDir = toolDir; // All D30 arms share the repaired core; A has no target spec.
const ledgerPath = resolve(process.env.D30_LEDGER ?? join(root, 'output/d30/budget.json'));
const expectedPlan = Object.fromEntries(['id','model','maxRequests','maxCostUsd','phases','price'].map(key => [key, study[key]]));
const readLedger = () => {
  const ledger=JSON.parse(readFileSync(ledgerPath,'utf8'));
  if(ledger.version!==1 || !Array.isArray(ledger.requests) || !isDeepStrictEqual(ledger.plan,expectedPlan)) throw new Error('Ledger does not match the exact frozen D30 plan');
  return ledger;
};
...
const nativePreflight=()=>{
 const path=join(root,'output/d30/native/preflight-status.json');
...
const offlinePreflight=()=>{
 const path=resolve(process.env.D30_OFFLINE_STATUS ?? join(root,'output/d30/offline/status.json'));
 const status=JSON.parse(readFileSync(path,'utf8'));
 if(!status.passed||status.providerCalls!==0||status.reportSha256!==hash(readFileSync(resolve(root,status.report))))throw new Error('D30 offline diagnostic missing or changed');
 for(const file of status.sourceManifest)if(hash(readFileSync(resolve(root,file.path)))!==file.sha256)throw new Error('D30 offline diagnostic source/build changed');
 return {path,sha256:hash(readFileSync(path)),...status};
};
const trackedInputs = () => [
  ...walk(join(root, 'src')), ...walk(toolDir), join(root,'evaluation/d29-audit-utils.mjs'),
  ...walk(join(root, 'evaluation/holdout')).filter(p => !p.includes('/output/') && !p.endsWith('native-results.json')),
  ...walk(join(root,'evaluation')).filter(p=>relative(join(root,'evaluation'),p).split('/').length===1 && /d30/.test(p)),
  join(root, 'evaluation/analyze-holdout.mjs'),
  join(root, 'docs/evaluation/scoped-d30-protocol.md'), join(root, 'package-lock.json'),
...
if (process.argv.includes('--freeze')) {
  ...
  const record = {id:study.id,createdAt:new Date().toISOString(),node:process.version,
    description:'Unchanged D29 core; 16 known conditions plus six frozen ordinary-change variants; explicit scope strata.',
    inputs:trackedInputs(),study,runtime:runtimeIdentity(),nativePreflight:nativePreflight(),offlinePreflight:offlinePreflight(),historicalProvenance:historicalProvenance(),ledgerPath,ledgerPlan:readLedger().plan,startingRequestIds:readLedger().requests.map(r=>r.id)};
  if(readLedger().requests.some(r=>r.phase==='holdout'))throw new Error('D30 synthetic phase must be unused');
...
if(!isDeepStrictEqual(offlinePreflight(),freeze.offlinePreflight)||!isDeepStrictEqual(historicalProvenance(),freeze.historicalProvenance))throw new Error('D30 diagnostics or historical provenance changed after freeze');
...
const {createCases} = await import('./d30-cases.mjs'); // only after freeze validation
...
for(let repeat=1;repeat<=study.holdout.repeats;repeat++) for(let index=0;index<cases.length;index++) {
  ...
  for(let j=0;j<3;j++) schedule.push({caseIndex:index,caseId:cases[index].id,group:cases[index].group,kind:cases[index].kind,scope:cases[index].scope,fresh:Boolean(cases[index].fresh),repeat,arm:['A','B','C'][(j+rotation)%3]});
}
if(schedule.length!==study.holdout.slots||schedule.length!==198)throw new Error('Invalid holdout slot count');
...
const records=schedule.map((slot,slotIndex)=>({...slot,slotIndex,expectation:cases[slot.caseIndex].expectation,startedAt:null,status:'unstarted',operational:null,run:null,assessment:null,requestHashes:[],requestPayloads:[]}));
...
try {
  ...
  for(let slotIndex=0;slotIndex<schedule.length;slotIndex++) {
    ...
    try {
      ...
      const provider={kind:underlying.kind,configuration:underlying.configuration,async select(input,signal){
        const payload=lib.serializeRequest(input,config);
        assertCleanPayload(payload);
        const requestSha256=hash(payload);
        record.requestHashes.push(requestSha256);
        record.requestPayloads.push({ordinal:record.requestPayloads.length+1,sha256:requestSha256,payload,payloadBytes:Buffer.byteLength(payload,'utf8'),coverage:structuredClone(input.coverage)});
        persist(); // exact sanitized bytes retained before any possible dispatch
  ...
  const frozenInputsIntact=JSON.stringify(trackedInputs())===JSON.stringify(freeze.inputs);
  const historicalInputsIntact=isDeepStrictEqual(historicalProvenance(),freeze.historicalProvenance);
  if(!frozenInputsIntact||!historicalInputsIntact)globalFailure=globalFailure??'FrozenInputsChanged';
  const groups=[];
  ...
  const summary={id:study.id,createdAt:new Date().toISOString(),globalFailure,frozenInputsIntact,historicalInputsIntact,planned:schedule.length,groups,parity,strata,holdoutAccounting:subtotal(holdoutAccounting),studyAccounting:subtotal(accounting.requests),
