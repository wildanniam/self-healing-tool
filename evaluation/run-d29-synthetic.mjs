...
const out = resolve(process.env.D29_SYNTHETIC_OUTPUT ?? join(root, 'output/d29/synthetic-v1'));
const toolDir = resolve(process.env.D29_TOOL_DIST ?? join(root, 'dist'));
const baselineDir = toolDir; // All D29 arms share the repaired core; A has no target spec.
const ledgerPath = resolve(process.env.D29_LEDGER ?? join(root, 'output/d29/budget.json'));
const expectedPlan = Object.fromEntries(['id','model','maxRequests','maxCostUsd','phases','price'].map(key => [key, study[key]]));
const readLedger = () => {
  const ledger=JSON.parse(readFileSync(ledgerPath,'utf8'));
  if(ledger.version!==1 || !Array.isArray(ledger.requests) || !isDeepStrictEqual(ledger.plan,expectedPlan)) throw new Error('Ledger does not match the exact frozen D29 plan');
  return ledger;
};
...
const nativePreflight=()=>{
 const path=join(root,'output/d29/native/preflight-status.json');
...
const offlinePreflight=()=>{
 const path=resolve(process.env.D29_OFFLINE_STATUS ?? join(root,'output/d29/offline/status.json'));
 const status=JSON.parse(readFileSync(path,'utf8'));
 if(!status.passed||status.providerCalls!==0||status.reportSha256!==hash(readFileSync(resolve(root,status.report))))throw new Error('D29 offline diagnostic missing or changed');
 for(const file of status.sourceManifest)if(hash(readFileSync(resolve(root,file.path)))!==file.sha256)throw new Error('D29 offline diagnostic source/build changed');
...
const trackedInputs = () => [
  ...
  ...walk(join(root,'evaluation')).filter(p=>relative(join(root,'evaluation'),p).split('/').length===1 && /d29/.test(p)),
  join(root, 'evaluation/analyze-holdout.mjs'),
  join(root, 'docs/evaluation/owner-evidence-protocol.md'), join(root, 'package-lock.json'),
...
if (process.argv.includes('--freeze')) {
  ...
  const record = {id:study.id,createdAt:new Date().toISOString(),node:process.version,
    description:'Owner evidence and bounded null refresh shared core with unchanged known synthetic regression fixtures; not an unseen holdout.',
  ...
  if(readLedger().requests.length!==0)throw new Error('D29 freeze requires its own unused ledger');
...
if(!isDeepStrictEqual(offlinePreflight(),freeze.offlinePreflight)||!isDeepStrictEqual(historicalProvenance(),freeze.historicalProvenance))throw new Error('D29 diagnostics or historical provenance changed after freeze');
