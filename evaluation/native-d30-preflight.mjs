...
check('twenty-two conditions',cases.length === 22);
check('two independent groups',new Set(cases.map(item=>item.group)).size === 2);
for (const group of new Set(cases.map(item=>item.group))) check('eleven conditions per group',cases.filter(item=>item.group===group).length === 11);
...
await mkdir(new URL('../output/d30/native/',import.meta.url),{recursive:true,mode:0o700});
await writeFile(new URL('../output/d30/native/'+reportName,import.meta.url),reportText,{flag:'wx',mode:0o600});
await writeFile(new URL('../output/d30/native/preflight-status.json',import.meta.url),JSON.stringify({passed:report.passed,checks:report.checks,report:'output/d30/native/'+reportName,reportSha256:sha(reportText),bundleSha256:report.fixtureBundleSha256,providerCalls:0,healerRuns:0},null,2)+'\n');
console.log(JSON.stringify({passed:report.passed,cases:report.cases,groups:report.groups,checks:report.checks,providerCalls:0,healerRuns:0,fixtureBundleSha256:report.fixtureBundleSha256,contractAuthorshipSha256:report.contractAuthorshipSha256,report:'output/d30/native/'+reportName},null,2));
