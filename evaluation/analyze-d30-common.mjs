...
function validateInputs(records, schedule, freeze, started, partial) {
  if (!Array.isArray(records) || !Array.isArray(schedule) || schedule.length !== 198) throw new Error('Expected the frozen 198-slot schedule and a results array.');
  ...
  if (records.length !== 198) throw new Error('Every analysis, including --partial, requires the full 198-slot inventory with predeclared expectation metadata. Missing records cannot be treated as absent opportunities.');
  const plan = freeze?.study?.holdout;
  if (!plan || plan.slots !== 198 || plan.cases !== 22 || plan.applicationGroups !== 2 || plan.repeats !== 3 || !isDeepStrictEqual(plan.arms, ARMS)) throw new Error('Unexpected frozen holdout design.');
  if (started?.slots !== 198 || started?.freezeSha256 !== hash(JSON.stringify(freeze))) throw new Error('started.json does not identify this exact freeze.json.');
  if (new Set(schedule.map(keyOf)).size !== 198 || new Set(schedule.map(row => row.caseId)).size !== 22 || new Set(schedule.map(row => row.group)).size !== 2) throw new Error('Frozen schedule identities are incomplete or duplicated.');
  const cases = new Map(schedule.filter(row => row.repeat === 1).map(row => [row.caseIndex, row]));
  for (let index = 0; index < 198; index++) {
    const repeat = Math.floor(index / 66) + 1, caseIndex = Math.floor((index % 66) / 3);
  ...
  for (const row of records) {
    if (!row || !integer(row.slotIndex) || row.slotIndex >= 198 || seen.has(row.slotIndex) || !isDeepStrictEqual(identity(row), identity(schedule[row.slotIndex]))) throw new Error('Result identity differs from the exact frozen schedule, or a slot is duplicated.');
  ...
  if (!partial && (records.length !== 198 || complete !== 198)) throw new Error(`Final analysis requires all 198 complete slots; found ${complete} complete in ${records.length} records. Use --partial only for explicitly partial output.`);
  return { expectedSlots: 198, records: records.length, complete, failed: count(records, r => r.status === 'failed'), unstarted: count(records, r => r.status === 'unstarted'), missingSlots: schedule.flatMap((_, i) => seen.has(i) ? [] : [i]), exactScheduleIdentity: true };
}
...
export function analyzeHoldout({ records, schedule, freeze, started, budget, partial = false, frozenSummary = null, provenance = {} }) {
  ...
  return { analysisVersion: 1, createdAt: new Date().toISOString(), status: partial ? 'explicitly-partial-posthoc' : 'final-collected-posthoc', completion, provenance,
    design: { applicationGroups: 2, caseVariants: 22, repeatsPerCaseArm: 3, arms: ARMS, plannedSlots: 198, inferentialUnits: 'Cases are nested variants within two application groups. Three repeats measure repeated runs; they are not independent tasks. No significance or population-generalization claims.' },
    ...
    definitions: {
      ...
      posthoc: 'Added descriptive state/acknowledgment breakdown. cleanStateGoalAttainment is reconstructed from retained state and history against frozen evaluator goals, never inferred from assessment.semantic. Every analysis requires the full 198-slot predeclared inventory. Opportunity denominators include unfinished slots; missing or unsupported observations remain unknown.',
...
export function renderMarkdown(report) {
  const lines = [`# D26 ${report.status === 'explicitly-partial-posthoc' ? 'PARTIAL — NOT FINAL' : 'completed collection'}: posthoc analysis`, '',
    `Completion: ${report.completion.complete}/198. Records: ${report.completion.records}. Exact frozen schedule identity verified.`, '',
    'Two application groups; 22 condition variants; three repeated runs per case and arm. Repeats are not independent tasks. No significance claims.', '',
