...
await mkdir(new URL('../output/d29/native/',import.meta.url),{recursive:true,mode:0o700});
await writeFile(new URL('../output/d29/native/'+reportName,import.meta.url),reportText,{flag:'wx',mode:0o600});
await writeFile(new URL('../output/d29/native/preflight-status.json',import.meta.url),JSON.stringify({passed:report.passed,checks:report.checks,report:'output/d29/native/'+reportName,reportSha256:sha(reportText),bundleSha256:report.fixtureBundleSha256,providerCalls:0,healerRuns:0},null,2)+'\n');
console.log(JSON.stringify({passed:report.passed,cases:report.cases,groups:report.groups,checks:report.checks,providerCalls:0,healerRuns:0,fixtureBundleSha256:report.fixtureBundleSha256,contractAuthorshipSha256:report.contractAuthorshipSha256,report:'output/d29/native/'+reportName},null,2));
