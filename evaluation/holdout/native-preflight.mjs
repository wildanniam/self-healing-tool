...
import { createCases, nativeReferences } from './cases.mjs';
...
const manifest = JSON.parse(await readFile(new URL('./evidence/contract-authorship.json',import.meta.url),'utf8'));
...
const report={kind:'native-preflight-only',startedAt,finishedAt:new Date().toISOString(),node:process.version,cases:cases.length,groups:new Set(cases.map(item=>item.group)).size,checks:checks.length,passed:!failure,providerCalls:0,healerRuns:0,networkRequests,fixtureBundleSha256:sha(JSON.stringify(files)),contractAuthorshipSha256:sha(await readFile(new URL('./evidence/contract-authorship.json',import.meta.url))),files,checkResults:checks,failure};
const reportName='native-preflight-'+startedAt.replaceAll(':','-')+'.json';
const reportText = JSON.stringify(report,null,2)+'\n';
await writeFile(new URL('./evidence/'+reportName,import.meta.url),reportText);
await writeFile(new URL('./preflight-status.json',import.meta.url),JSON.stringify({passed:report.passed,checks:report.checks,report:'evaluation/holdout/evidence/'+reportName,reportSha256:sha(reportText),bundleSha256:report.fixtureBundleSha256,providerCalls:0,healerRuns:0},null,2)+'\n');
console.log(JSON.stringify({passed:report.passed,cases:report.cases,groups:report.groups,checks:report.checks,providerCalls:0,healerRuns:0,fixtureBundleSha256:report.fixtureBundleSha256,contractAuthorshipSha256:report.contractAuthorshipSha256,report:'evaluation/holdout/evidence/'+reportName},null,2));
