...
async function main() {
  ...
  let directory = join(root, 'output/d29/synthetic-v1'), budgetPath, partial = false;
  for (let i = 0; i < args.length; i++) {
    ...
    } else throw new Error('Usage: node evaluation/analyze-d29-synthetic.mjs [--directory PATH] [--budget PATH] [--partial]');
  }
  ...
  for(const caseId of new Set(inputs.records.map(row=>row.caseId)))for(const arm of ['A','B','C']){
    ...
    conditionRows.push({caseId,arm,kind:rows[0].kind,planned:rows.length,complete:complete.length,
  ...
  report.d29ExactRequests={captured:requestChecks.length,allHashesVerified:true,attempts:requestChecks};
  report.d29PerCondition=conditionRows;
  report.d29HistoricalInputsUnchanged=isDeepStrictEqual(historicalProvenance(),inputs.freeze.historicalProvenance);
  if(!report.d29HistoricalInputsUnchanged)throw new Error('D27 or frozen fixture inputs changed');
  report.d29Budget={maxCostUsd:inputs.budget.plan.maxCostUsd,maxRequests:inputs.budget.plan.maxRequests,reservedUsd:inputs.budget.requests.reduce((sum,row)=>sum+row.reservedUsd,0),allReservationsWithinCap:inputs.budget.requests.reduce((sum,row)=>sum+row.reservedUsd,0)<=inputs.budget.plan.maxCostUsd};
  if(!report.d29Budget.allReservationsWithinCap)throw new Error('D29 reservation cap exceeded');
  report.limitations=report.limitations.map(x=>x.replace('Frozen methods and primary metrics unchanged. State/acknowledgment analyses were added after inspecting outcomes.','Primary and supplementary state/acknowledgment analyses inherited from D26 and specified before D29 collection.').replace('Synthetic holdout only; no real-world or universal-effectiveness claim.','Known synthetic regression rerun: previously inspected cases, not unseen validation.'));
  ...
  await writeFile(markdownPath, renderMarkdown(report).replace('# D26 ', '# D29 ').replace('## Added posthoc state', '## State') + '\n## D29 exact request audit\n\nEvery captured request hash was compared with its per-attempt input hash, recorded coverage and reservation payload bytes. Initial event context was not substituted for refreshed attempts. See `d29ExactRequests`, `d29PerCondition`, `d29Budget` and `d29HistoricalInputsUnchanged` in the JSON report.\n', { flag: 'wx', mode: 0o600 });
