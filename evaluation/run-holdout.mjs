...
const study = JSON.parse(readFileSync(join(root, 'evaluation/d26-config.json'), 'utf8'));
const out = resolve(process.env.D26_HOLDOUT_OUTPUT ?? join(root, 'output/d26/holdout-v1'));
const baselineDir = resolve(process.env.D26_BASELINE_DIST ?? '/private/tmp/self-healing-core-005/dist');
const toolDir = resolve(process.env.D26_TOOL_DIST ?? join(root, 'dist'));
const ledgerPath = resolve(process.env.D26_LEDGER ?? join(root, 'output/d26/budget.json'));
const expectedPlan = Object.fromEntries(['id','model','maxRequests','maxCostUsd','phases','price'].map(key => [key, study[key]]));
const readLedger = () => {
  const ledger=JSON.parse(readFileSync(ledgerPath,'utf8'));
  if(ledger.version!==1 || !Array.isArray(ledger.requests) || !isDeepStrictEqual(ledger.plan,expectedPlan)) throw new Error('Ledger does not match the exact frozen D26 plan');
  return ledger;
};
...
const nativePreflight=()=>{
 const path=join(root,'evaluation/holdout/preflight-status.json');
...
const trackedInputs = () => [
  ...walk(join(root, 'src')), ...walk(toolDir), ...walk(baselineDir),
  ...walk(join(root, 'evaluation/holdout')).filter(p => !p.includes('/output/') && !p.endsWith('native-results.json')),
  join(root, 'evaluation/run-holdout.mjs'), join(root, 'evaluation/d26-config.json'),
  join(root, 'docs/evaluation/spec-aware-protocol.md'), join(root, 'package-lock.json'),
...
if (process.argv.includes('--freeze')) {
  ...
  const record = {id:study.id,createdAt:new Date().toISOString(),node:process.version,
    description:'Core, contracts, independent holdout and protocol frozen before any held-out provider result.',
...
try {
  ...
  const summary={id:study.id,createdAt:new Date().toISOString(),globalFailure,planned:schedule.length,groups,parity,strata,holdoutAccounting:subtotal(holdoutAccounting),studyAccounting:subtotal(accounting.requests),
    ...
    limitations:['Synthetic holdout, not real-world generalization','Three repeats are not independent tasks','Contracts manually authored before mutations','Lexical admission does not prove business semantics','Do not tune the frozen batch after inspecting outcomes']};
