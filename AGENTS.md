...
Latest verified checkpoint: D33 continuation collection is COMPLETE/CLOSED: 9 pilot and 108 main executions, all 117 new requests settled, primary and independent audits pass. Read `docs/evidence/rm1-continuation-results-2026-09-18.md`. Main R0/R2 each 36/36 correct; R1 30/36, 3 wrong, 3 refusals; no full-ranker advantage over lexical baseline. Preserve original 50 and continuation 57 frozen inputs, all ledgers, and pinned pilot files (do not regenerate pilot analysis after main freeze). Historical failed cost stays null; combined 119 requests with 118 known. No further paid calls, repeated studies, merge, release or submission follow from unused allowance.

Historical authorization checkpoint (collection is now closed): Wildan explicitly authorized analysis and execution after the pilot continuation was explained. Apply `docs/evaluation/rm1-continuation-protocol.md`. It supersedes the prior exact-old-charge prerequisite for this acknowledged historical incident only: retain the original halted ledger/unknown cost and both reservations, cap companion execution at349requests/USD2.99816535, verify explicit Vanie routing, and halt on any new unknown. Preserve all50 original frozen inputs; do not reset ledgers or claim billing reconciliation.

Historical interruption checkpoint (superseded by the completed continuation above): D33 RM1 implementation and offline selection analysis are verified under issue25; the paid pilot stopped after its first transport failure with unknown usage. Read `docs/evidence/rm1-ranking-2026-09-18.md`. Preserve the halted ledger and frozen50files; eight pilot and108main slots remain unstarted. R0 and R2 both retain12/12offline targets, R1 retains10/12; these are not model recovery results. Unknown cost/tokens are null, not zero. Frozen analyzer zero placeholders after audit FAIL must not be used as results; use `evaluation/rm1/report-partial.mjs`. Continuation requires genuine usage reconciliation and a documented resume protocol, preserving the total US$3/351request cap and failed-slot lineage. Do not reset or silently replace the ledger.
