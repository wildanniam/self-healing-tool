...
D35 report extension (20 September, issue #29): the owner authorizes a packaged shared action inspector and English/Indonesian UI. Read `docs/evidence/bilingual-library-report-2026-09-20.md`, `docs/integration.md` (local audit reports) and `DESIGN.md`. `audit: true` captures adapter-dispatched bodies and returned text separately from snapshots; supply `session.audit()` explicitly to `writeReport`. Safe summaries stay payload-free. The shared UI lives in `src/report-inspector.js`, `report-language.js` and `report-style.js`; compile before presentation tests/CLI. Translate authored labels only, never evidence. Preserve exact attempt identity, redaction status and missing historical sources. No method/data/paper changes, paid inference, merge or release. This supersedes only D34's restriction on optional rich library reports. The paper describes final capabilities, not migration history.

Presentation checkpoint D34 (18 September): issue #27 simplifies both report paths with result-first summaries and four evidence groups. Read `DESIGN.md`, `docs/presentation.md` and `docs/evidence/report-simplification-2026-09-18.md`. `--ranking` / local `rankingDirectory` reads existing D33 main records into a separate RM1 view; it does not run experiments or change the D30 core. Summary JSON and CSV follow identical active case filters; filtered snapshots retain scope metadata. Exact run/attempt identity must survive navigation. Keep missing raw outputs, model null, controls, replay and unassessed actions explicit. Do not expand the generic safe report projection to obtain UI parity. The work is stacked on D32 / PR #24, independently of the D33 runtime branch; no merge or paid run is authorized by this checkpoint.
