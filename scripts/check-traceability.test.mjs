...
function fixture(t) {
  ...
  for (const path of ['README.md', 'AGENTS.md', 'DESIGN.md', '.env.example', 'docs', 'openspec', '.github', 'src', 'tests', 'examples', 'scripts', 'package.json', 'tsconfig.json', 'playwright.config.ts']) cpSync(join(source, path), join(root, path), { recursive: true });
  return root;
}
