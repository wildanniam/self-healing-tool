...
const env = Object.fromEntries(['PATH', 'HOME', 'TMPDIR', 'CI', 'PLAYWRIGHT_BROWSERS_PATH'].filter(k => process.env[k]).map(k => [k, process.env[k]]));
try {
  ...
  const allowed = /^(dist\/[a-z-]+\.(?:js|d\.ts)|README\.md|package\.json|docs\/integration\.md|docs\/implementation\/component-inventory\.md)$/;
  ...
  execFileSync('npm', ['install', '--offline', '--ignore-scripts', '--no-audit', '--no-fund', '--package-lock=false', join(directory, packed.filename), 'playwright@1.62.1'], { cwd: consumer, env, stdio: 'pipe' });
