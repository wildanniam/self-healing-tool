...
Task 8.1 is complete for this reviewed release candidate. Task 8.3's prepublication review is complete: [issue #35](https://github.com/wildanniam/self-healing-tool/issues/35), branch `codex/35-public-release`, [PR #36](https://github.com/wildanniam/self-healing-tool/pull/36), D37 proposal/design/decision, requirement/scenario evidence and the owner's MIT/public-release authorization are connected. The private Atlas project hub and session note record the current candidate, PR #34 merge, PR #36 and remaining deployment checks. The audit checker and its seven regression checks pass; the evidence-fixture test preserves existing evidence needed by other completed tasks. Governance scenarios are checked through both those negative fixtures and direct review of the dated decision/PR/memory trail. The older paper/practitioner tasks remain open; the OpenSpec change is not archived. Public visibility and anonymous installation are not yet claimed by this candidate record. npm publication is outside the authorized scope.
