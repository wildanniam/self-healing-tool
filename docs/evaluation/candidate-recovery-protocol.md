...
Pre-freeze review additionally requires generic privacy/positional-selector regressions and bounded DOM work. These safeguards apply to all arms. Snapshot sanitization can redact subsequently known fill values from retained context but cannot retract earlier provider transmissions. Original coverage/hash measurements are retained as provenance, not recomputed from a later redacted view.
